...
//********************************************
	...
	for( i = 0; i < Height; i++ )
      	for( j = 0; j < Width; j++ )
            for( k = 0; k < 3; k++ )
		    Raster[j*BytesPerRow+i+k] = c_aux;
//********************************************
...
void bmpman :: Delete()
{
     ...
     cout << "DELETING CONTENTS OF THE IMAGE ..." << endl;
...
RGBQUAD bmpman :: GetRGBdata( int x, int y )
{
    ...
    if( BPP == 24 ){
        ...
        rgb_aux.rgbRed = Raster[y*BytesPerRow + x + 2];//2 = rgbRed;
        rgb_aux.rgbGreen = Raster[y*BytesPerRow + x + 1];//1 = rgbGreen;
        rgb_aux.rgbBlue = Raster[y*BytesPerRow + x];//0 = rgbBlue;
...
void bmpman :: EnterRGBdata( int x, int y, RGBQUAD rgb )
{
    ...
    if( BPP == 24 ){
        Raster[y*BytesPerRow + x + 2] = rgb.rgbRed;
        Raster[y*BytesPerRow + x + 1] = rgb.rgbGreen;
        Raster[y*BytesPerRow + x] = rgb.rgbBlue;
    }
    else
        cout << "ERROR, IMAGE IS NOT 24BPP" << endl;
}
