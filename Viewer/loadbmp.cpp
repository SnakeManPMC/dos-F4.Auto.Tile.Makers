					...
					LPSTR lpCmdLine, int nCmdShow )
{
	...
	hwnd = CreateWindow (szAppName,"Falcon 4.0 Tile viewer. <Com_GP@hotmail.com> based on proggie by Juan Soulie",WS_OVERLAPPEDWINDOW,
...
//  int Times = 6.25 ~ 7 para 800x600
	...
	{
		case WM_CREATE:
			bmp.LoadBMP ("Tile.bmp");
			return 0;
		case WM_PAINT:
			hdc=BeginPaint (hwnd,&ps);
			bmp.GDIPaint (hdc,0,0);
			bmp.GDIPaint (hdc,bmp.Width,bmp.Height);

            Times = (1024/bmp.Width);

            for( int i = 0; i < Times; i++ )
                 for( int j = 0; j < Times*0.75; j++ )
			         bmp.GDIPaint (hdc,i*bmp.Width,j*bmp.Height);
			EndPaint (hwnd,&ps);
			return 0;
		case WM_DESTROY:
			PostQuitMessage (0);
...
int CRaster::LoadBMP (char * szFile)
{
	...
	if (bmfh.bfType!=WORD('MB')) return 2;		// File is not BMP
