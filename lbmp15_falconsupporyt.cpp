...
class CRaster {
	public:
		...
		void LoadBMP (string szFile);
  		void SaveBMP (string szFile, CRaster bmp, int temp_i );
...
void CRaster::LoadBMP (string szFile)
{
...
void SaveBMP (string szFile, CRaster bmp, int temp_i )
{
    ...
    bmpfile1.open(szFile.c_str());

	if (!bmpfile1)
        cout << "ERROR SAVING FILE" << endl;		// Error opening file

	//Save bitmap fileheader & infoheader
	bmpfile1.write ((char*)&bmfh,sizeof (BITMAPFILEHEADER));
	bmpfile1.write ((char*)&bmih,sizeof (BITMAPINFOHEADER));
...
//	bmpfile.write ((char*)&bmp.Palette,256*sizeof (RGBQUAD) * (1<<bmp.BPP));
    ...
    for( i = 0; i < 256; i++ )
    {
    bmpfile1.write ((char*)&bmp.Palette[i], sizeof(char)*4);
//    cout << i << endl;
    }
...
//    system("PAUSE");
    ...
    for( i = (bmp.Height-1); i >= 0; i-- )
        ...
        {
             bmpfile1.write ((char*)&bmp.Raster[i][j], sizeof(char));
//             cout << i << ", " << j << endl;
        }
...
//    char ** Raster_aux, ** Raster_main;        //Points wherever the image bytes start
  ...
  cout << "Name of the file you want to open (EX: 512x512.bmp)\n";
  cin >> cc;
  cout << endl;
